...
import cvzone
import math
from playsound import playsound  # Import playsound for sound

# Define the path to the buzzer sound file
buzzer_sound_path = "D:/C DRIVE/Downloads/Project/ppe_detection/buzzer.mp3"  # Replace with your buzzer sound file path
...
# Flag to ensure the buzzer plays only once per alert
        ...
        for box in boxes:
            ...
            if conf > 0.5:
                if currentClass in ['NO-Hardhat', 'NO-Safety Vest', 'NO-Mask', 'NO-Person', 'NO-Gloves']:
                    if not buzzer_playing:  # Play buzzer only if not already playing
                        buzzer_playing = True
                        playsound(buzzer_sound_path)
                        buzzer_playing = False  # Reset flag after playing sound
